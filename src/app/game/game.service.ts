...
import { King, Pawn, Piece } from './piece';
...
export class GameService {
  ...
  constructor() {
    this._redTurnBeh = new BehaviorSubject(true) as BehaviorSubject<boolean>;
    this._resetGame = new BehaviorSubject(true) as BehaviorSubject<boolean>;
    this._isWinner = new BehaviorSubject('none') as BehaviorSubject<string>;
    this.resetGame();
  }
  ...
  calcDiag(p: Piece, up: boolean, right: boolean) {
    ...
    if (up) {
      if (right) {
        neighborRow = (p as Pawn).getUpRightMove().row;
        neighborCol = (p as Pawn).getUpRightMove().col;
        diagRow = (p as Pawn).getDiagUpRightMove().row;
        diagCol = (p as Pawn).getDiagUpRightMove().col;
      }
      if (!right) {
        neighborRow = (p as Pawn).getUpLeftMove().row;
        neighborCol = (p as Pawn).getUpLeftMove().col;
        diagRow = (p as Pawn).getDiagUpLeftMove().row;
        diagCol = (p as Pawn).getDiagUpLeftMove().col;
      }
    } else if (!up && p.type === 'king') {
      if (right) {
        neighborRow = (p as King).getDownRightMove().row;
        neighborCol = (p as King).getDownRightMove().col;
        diagRow = (p as King).getDiagDownRightMove().row;
        diagCol = (p as King).getDiagDownRightMove().col;
      }
      if (!right) {
        neighborRow = (p as King).getDownLeftMove().row;
        neighborCol = (p as King).getDownLeftMove().col;
        diagRow = (p as King).getDiagDownLeftMove().row;
        diagCol = (p as King).getDiagDownLeftMove().col;
      }
    }
